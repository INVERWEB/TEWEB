...
def leer_enterprise_sqlite(db_path):
    con = sqlite3.connect(db_path)
    df = pd.read_sql_query("SELECT * FROM enterprise_values_plana", con)
    con.close()
    return df
...
def convertir_columnas_numericas(df):
    columnas_numericas = [
        ...
        'numberOfShares', 'enterpriseValue'
    ]
...
def insertar_a_postgres(df, tabla_destino, engine):
    ...
    columnas_permitidas = [
        'ticker', 'anio', 'marketCapitalization', 'addTotalDebt',
        'minusCashAndCashEquivalents', 'numberOfShares', 'enterpriseValue'
    ]
...
def main():
    ...
    tabla_sqlite = "enterprise_values_plana"
    tabla_postgres = "enterprise_values_plana"

    print("📥 Leyendo registros desde SQLite...")
    df_raw = leer_enterprise_sqlite(ruta_db)
