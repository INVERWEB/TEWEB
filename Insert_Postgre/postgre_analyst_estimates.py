...
def leer_analyst_estimates_sqlite(db_path):
    con = sqlite3.connect(db_path)
    df = pd.read_sql_query("SELECT * FROM analyst_estimates_plana", con)
    con.close()
    return df
...
def insertar_a_postgres(df, tabla_destino, engine):
    ...
    columnas_permitidas = [
        "ticker", "anio", "estimatedRevenueLow", "estimatedRevenueHigh", "estimatedRevenueAvg",
        "estimatedEbitdaLow", "estimatedEbitdaHigh", "estimatedEbitdaAvg",
        "estimatedEbitLow", "estimatedEbitHigh", "estimatedEbitAvg",
        "estimatedNetIncomeLow", "estimatedNetIncomeHigh", "estimatedNetIncomeAvg",
        "estimatedSgaExpenseLow", "estimatedSgaExpenseHigh", "estimatedSgaExpenseAvg",
...
def main():
    ...
    tabla_sqlite = "analyst_estimates_plana"
    tabla_postgres = "analyst_estimates_plana"

    print("📥 Leyendo registros desde SQLite...")
    df_raw = leer_analyst_estimates_sqlite(ruta_db)
